...
import (
	...
	"github.com/guapcrypto/rosetta-guapcoin/indexer"
...
func startOnlineDependencies(
	...
	g *errgroup.Group,
) (*guap.Client, *indexer.Indexer, error) {
	client := guap.NewClient(
		guap.LocalhostURL(cfg.RPCPort),
		cfg.GenesisBlockIdentifier,
		cfg.Currency,
	)

	g.Go(func() error {
		return guap.Startguapcoind(ctx, cfg.ConfigPath, g)
	})
...
func main() {
	...
	if signalReceived {
		logger.Fatalw("rosetta-guap halted")
	}

	if err != nil {
		logger.Fatalw("rosetta-guap sync failed", "error", err)
	}
}
