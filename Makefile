...
GO_PACKAGES=./services/... ./indexer/... ./guap/... ./configuration/...
...
build:
	docker build -t rosetta-guap:latest https://github.com/guapcrypto/rosetta-guapcoin.git

build-local:
	docker build -t rosetta-guap:latest .


build-release:
	# make sure to always set version with vX.X.X
	docker build -t rosetta-guap:$(version) .;
	docker save rosetta-guap:$(version) | gzip > rosetta-guap-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/guap-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 9633:9633 rosetta-guap:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-guap:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/guap-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 19246:19246 rosetta-guap:latest

run-testnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=TESTNET" -e "PORT=8081" -p 8081:8081 rosetta-guap:latest
