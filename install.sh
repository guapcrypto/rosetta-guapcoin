...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for guapcrypto/rosetta-guapcoin
...
execute() {
  ...
  docker tag "rosetta-guap:${TAG}" "rosetta-guap:latest"
  log_info "loaded rosetta-guap:${TAG} and tagged as rosetta-guap:latest"
  rm -rf "${tmpdir}"
